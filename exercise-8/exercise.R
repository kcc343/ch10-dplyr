...
circulation_data <- read.csv("data/pulitzer-circulation-data.csv", 
...
ciruculation_change <- circulation_data %>% mutate(
  Pulitzer.Prize.Change = Pulitzer.Prize.Winners.and.Finalists..2004.2014 -
...
winner_decrease <- ciruculation_change %>% 
  filter(Change.in.Daily.Circulation..2004.2013 >= 5, 
         Change.in.Daily.Circulation..2004.2013 == 
           min(Change.in.Daily.Circulation..2004.2013)) %>% 
  arrange(Pulitzer.Prize.Change)
